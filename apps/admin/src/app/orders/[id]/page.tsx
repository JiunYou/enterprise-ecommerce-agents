...
export default async function AdminOrderDetailPage({ params }: OrderDetailPageProps) {
  ...
  // 2. 解析 Next.js 16 Promise-based params
  ...
  // 5. 處理權限不足（403 Forbidden）狀態：嚴格不洩漏任何訂單資料與 PII
    ...
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-zinc-50 px-4 dark:bg-zinc-950">
        <div className="w-full max-w-lg rounded-xl border border-rose-200 bg-rose-50/50 p-6 sm:p-8 text-center shadow-xs dark:border-rose-900/50 dark:bg-rose-950/20">
          ...
          <p className="mt-2 text-sm text-zinc-600 dark:text-zinc-400">
            當前登入之帳號未具備系統管理員 (Admin) 權限，無法存取訂單詳細資訊。
          </p>
  ...
  return (
    <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950">
      ...
      <AdminHeader
        ...
        title="訂單明細檢視 (Admin Order Detail)"
        subtitle="唯讀訂單詳情與收件資訊"
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
            ...
            <div className="rounded-xl border border-zinc-200 bg-white p-5 sm:p-6 shadow-xs dark:border-zinc-800 dark:bg-zinc-900">
              ...
              {order.items.length === 0 ? (
                <p className="mt-3 text-xs text-zinc-500 dark:text-zinc-400">此訂單尚無商品項目。</p>
              ) : (
                  ...
                  <div className="flex items-start gap-2.5">
                    ...
                    <div>
                      ...
                      <p className="mt-1 text-amber-700 dark:text-amber-400">
                        此訂單無快照之配送地址資訊（例如早期歷史建立之訂單或尚未填寫配送資訊之狀態）。
                      </p>
                    </div>
                  </div>
                </div>
