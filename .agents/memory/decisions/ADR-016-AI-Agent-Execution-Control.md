...
1. **明確的任務邊界 (Explicit Task Boundary)：** 每個任務必須明確定義「允許修改範圍 (Allowed Files)」與「禁止更動範圍 (Forbidden Files)」。
2. **明確定義的完成條件 (Defined Completion Criteria)：** 具備可驗證的交付標準，一旦達成驗收條件，Agent 必須「立即停止」(STOP IMMEDIATELY)。
3. **強制迭代上限 (Maximum Iteration Limit)：** 單一問題修復上限為 3 次，若超過則強制中止並交由人工介入分析。
