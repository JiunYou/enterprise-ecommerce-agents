...
public class ProductCategoryMigrationAcceptanceTests : IAsyncLifetime
{
    private const string DatabaseName = "product_category_migration_test_db";
    private readonly MySqlContainer _mySqlContainer;

    public ProductCategoryMigrationAcceptanceTests()
    {
    ...
    private static (string PreviousMigration, string TargetMigration) ResolveMigrations(EnterpriseCommerceDbContext dbContext)
    {
        ...
        var targetMigration = allMigrations.FirstOrDefault(m => m.Contains("AddProductCategory"));
        if (targetMigration is null)
        {
            throw new InvalidOperationException("Migration AddProductCategory does not exist yet.");
        }
        var targetIndex = allMigrations.IndexOf(targetMigration);
        if (targetIndex <= 0)
        {
            throw new InvalidOperationException("AddProductCategory is the first migration or index invalid.");
        }
    ...
    [Fact]
    public async Task MIGRATION_UPGRADE_PREVIOUS_TO_PRODUCT_CATEGORY_Passes()
    {
        ...
        // 1. 套用至上一遷移 (AddProductImageUrl)
        var migrator = dbContext.GetInfrastructure().GetRequiredService<IMigrator>();
        await migrator.MigrateAsync(previousMigration);

        var conn = dbContext.Database.GetDbConnection();
        var colsBefore = await GetProductColumnsAsync(conn);
        colsBefore.Should().NotContainKey("Category");
        colsBefore.Should().ContainKey("ImageUrl");

        // 2. 在舊 schema 下插入商品列 (包含 ImageUrl)
        var productId = Guid.NewGuid();
        var productName = "Historical Product";
        var productSku = "HIST-SKU-CAT-001";
        var productPrice = 99.99m;
        var productCurrency = "USD";
        var productDescription = "Historical Description";
        var productImageUrl = "https://example.com/historical.jpg";

        await using (var insertCmd = conn.CreateCommand())
        {
            insertCmd.CommandText = @"
                INSERT INTO Products (Id, Name, Sku, Price, Currency, IsActive, Description, ImageUrl, Version)
                VALUES (@id, @name, @sku, @price, @currency, 1, @description, @imageUrl, 1);";
            ...
            var pImg = insertCmd.CreateParameter();
            pImg.ParameterName = "@imageUrl";
            pImg.Value = productImageUrl;
            insertCmd.Parameters.Add(pImg);

            await insertCmd.ExecuteNonQueryAsync();
        }

        // 3. 套用目標遷移 AddProductCategory
        await migrator.MigrateAsync(targetMigration);

        var colsAfter = await GetProductColumnsAsync(conn);
        colsAfter.Should().ContainKey("Category");
        colsAfter["Category"].IsNullable.Should().Be("NO");
        colsAfter["Category"].MaxLen.Should().Be(100);

        // 4. 驗證既有商品存活且 Category 為預設空字串，其餘欄位不變
        await using (var queryCmd = conn.CreateCommand())
        {
            queryCmd.CommandText = "SELECT Id, Name, Sku, Price, Currency, IsActive, Description, ImageUrl, Category FROM Products WHERE Id = @id;";
            ...
            reader.GetString(7).Should().Be(productImageUrl);
            reader.GetString(8).Should().Be(string.Empty);
        }
    ...
    public async Task MIGRATION_FRESH_DATABASE_Passes()
    {
        ...
        cols.Should().ContainKey("Category");
        cols["Category"].IsNullable.Should().Be("NO");
        cols["Category"].MaxLen.Should().Be(100);
    ...
    [Fact]
    public async Task MIGRATION_DOWN_PRODUCT_CATEGORY_TO_PREVIOUS_Passes()
    {
        await using var dbContext = CreateDbContext();
        await ResetDatabaseAsync(dbContext);
        ...
        colsBeforeDown.Should().ContainKey("Category");

        // 降級至上一遷移
        await migrator.MigrateAsync(previousMigration);

        var colsAfterDown = await GetProductColumnsAsync(conn);
        colsAfterDown.Should().NotContainKey("Category");
        ...
        colsAfterDown.Should().ContainKey("ImageUrl");
